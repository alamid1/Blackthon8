...
async def _(event):
    await event.edit(
        """
أوامر الصيد الخاصة بسورس بـلاگ ثـون : 
    ...
    try:
        ch = await sython(
            functions.channels.CreateChannelRequest(
                title="صيد بـلاگ ثـون",
                about="تاج راسي بـلاگ ثـون @SpAnEi",
            )
        )
        ...
        if isav == True:
            try:
                ...
                await event.client.send_file(
                    event.chat_id,
                    "https://t.me/ViPALsultan/2",
                    caption="🐊 BLackthon the best 🐊\n- - - - - - - - - - - - - - - - - - - - - - - -\n- UserName: ❲ @{} ❳\n- ClickS: ❲ {} ❳\n- Type: {}\n- Save: ❲ Chaneel ❳\n- - - - - - - - - - - - - - - - - - - - - - - -\nThE BLackthon ❲ @BLackthon1 - @SpAnEi ❳ ".format(
                ...
                await event.client.send_message(
                    "@ViPL0Z", f"- Done : @{username} !\n- By : @SpAnEi - @BLackthon1 !"
                )
                sedmod = False
                break
...
async def _(event):
    ...
    try:
        ...
        try:
            ch = await sython(
                functions.channels.CreateChannelRequest(
                    title="تثبيت بـلاگ ثـون",
                    about="تثبيت بـلاگ ثـون @SpAnEi",
                )
            )
            ch = ch.updates[1].channel_id
